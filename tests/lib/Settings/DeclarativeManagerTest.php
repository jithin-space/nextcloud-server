...
class DeclarativeManagerTest extends TestCase {
	...
	static $testSetInternalValueAfterChange = false;
	...
	public function testRegisterSchema(): void {
		...
		$formIds = $this->declarativeManager->getFormIDs($schema['section_type'], $schema['section_id']);
	...
	public function testRegisterMultipleSchemasAndDuplicate(): void {
		...
		$formIds = $this->declarativeManager->getFormIDs($schemaDuplicateFields['section_type'], $schemaDuplicateFields['section_id']);
	...
	public function testGetFormsWithDefaultValues(): void {
		...
		// Mock user with admin rights as we're testing form in admin section
		...
		$forms = $this->declarativeManager->getFormsWithValues($user, $schema['section_type'], $schema['section_id']);
	...
	public function testGetFormsWithDefaultValuesJson(): void {
		...
		$this->config->expects($this->once())
			->method('getUserValue')
			->with($user->getUID(), $app, 'test_field_json', json_encode($schema['fields'][0]['default']))
	...
	public function testAdminFormUserUnauthorized(): void {
		...
		// Mock user without admin rights as we're testing form in admin section
		$user = $this->createMock(IUser::class);
		$user->expects($this->any())
			->method('getUID')
			->willReturn('test_user');
		...
		$this->expectException(\Exception::class);
		$this->declarativeManager->getFormsWithValues($user, $schema['section_type'], $schema['section_id']);
	}
}
